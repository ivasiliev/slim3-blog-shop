 ...
 */
if (!$inWidget)
    die('inWidget object was not initialised.');
if (!is_object($inWidget->data))
    die('<b style="color:red;">Cache file contains plain text:</b><br />' . $inWidget->data);
?>
<!DOCTYPE html> 
<html lang="ru">
    <head>
	...
	<div id="widget" class="widget">
	    <a style="display: none;" href="http://instagram.com/<?php echo $inWidget->data->username; ?>" target="_blank" class="title">
		<img 
		...
		foreach ($inWidget->data->images as $key => $item) {
		    ...
		    echo '<a class="img_elem_show" href="' . $item->link . '" style="background-image:url(' . $thumbnail . ');" target="_blank">&nbsp;</a>';
		    $i++;
		    if ($i >= $inWidget->view)
			break;
		}
		...
		if (!empty($inWidget->config['HASHTAG'])) {
		    $inWidget->lang['imgEmptyByHash'] = str_replace(
			    '{$hashtag}', $inWidget->config['HASHTAG'], $inWidget->lang['imgEmptyByHash']
		    );
